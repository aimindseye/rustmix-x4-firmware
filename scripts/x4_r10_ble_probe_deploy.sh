...
export R10_BLE_X4_MODE="$MODE"
export R10_BLE_X4_ALLOW_READER_REMOTE="${R10_BLE_X4_ALLOW_READER_REMOTE:-0}"
...
BUILD_MODE_RS="target-xteink-x4/src/rustmix_x4/ring_remote/r10_ble_x4_build_mode.rs"

cat > "$BUILD_MODE_RS" <<EOF
// Generated by scripts/x4_r10_ble_probe_deploy.sh.
// Do not edit manually during a deploy run.

pub const R10_BLE_X4_COMPILED_MODE: &str = "$MODE";
pub const R10_BLE_X4_COMPILED_ALLOW_READER_REMOTE: &str = "${R10_BLE_X4_ALLOW_READER_REMOTE:-0}";
pub const R10_BLE_X4_COMPILED_SETTINGS: &str = "${R10_BLE_X4_SETTINGS:-1}";
...
echo "==> Generated R10 BLE compile mode source:"
grep "^pub const R10_BLE_X4_COMPILED_" "$BUILD_MODE_RS"
...
cargo test -p "$PKG" r10_ble_live_central -- --nocapture
...
export R10_BLE_X4_SETTINGS="${R10_BLE_X4_SETTINGS:-1}"
echo "==> Cleaning ESP firmware target for deploy mode: $MODE"
cargo clean -p "$PKG" --target "$TARGET"

echo "==> Checking firmware with feature: $FEATURE"
cargo check --release -p "$PKG" --features "$FEATURE" --target "$TARGET"

echo "==> Building release firmware for target: $TARGET"
cargo build --release -p "$PKG" --features "$FEATURE" --target "$TARGET"

ELF="${R10_BLE_X4_ELF:-}"
if [[ -z "$ELF" ]]; then
  ELF="$(find "target/$TARGET/release" -maxdepth 1 -type f -name "$PKG" -perm -111 2>/dev/null | sort | tail -n 1 || true)"
fi

if [[ -z "$ELF" || ! -f "$ELF" ]]; then
  echo "Could not locate release ELF for $PKG under target/$TARGET/release/$PKG" >&2
