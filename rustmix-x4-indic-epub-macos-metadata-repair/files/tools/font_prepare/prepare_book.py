...
def run_prepared_txt_generator(
    *,
    ...
    if returncode != 0:
        raise RuntimeError(
            f"prepared cache generator failed with exit status {returncode}; "
    ...
    return PreparedResult(book_id=book_id, cache_dir=cache_dir, stdout=output)
...
def is_macos_metadata(name: str) -> bool:
    """Return True for Finder/AppleDouble files that are not cache assets."""

    return name == ".DS_Store" or name.startswith("._")


def remove_macos_metadata(cache_dir: Path | None) -> int:
    """Remove FAT-volume sidecars produced by macOS below one cache directory.

    macOS may create AppleDouble `._*` files while writing directly to a FAT SD
    card. They are not Reader assets and must not be copied or validated as
    firmware inputs. The deterministic page loader never opens them, but
    scrubbing them keeps the SD cache clean and avoids confusing diagnostics.
    """
    ...
    for path in sorted(cache_dir.rglob("*"), key=lambda item: len(item.parts), reverse=True):
        if path.is_file() and is_macos_metadata(path.name):
            path.unlink(missing_ok=True)
            removed += 1
    if removed:
        print(f"removed_macos_metadata={removed}")
    return removed
...
def validate_cache(cache_dir: Path, device_path: str | None) -> int:
    ...
    remove_macos_metadata(cache_dir)
...
def print_upload_instructions(cache_dir: Path | None, book_id: str | None) -> None:
    ...
    files = sorted(
        ...
        if path.is_file() and not is_macos_metadata(path.name)
    )
    print()
    print("SD upload:")
