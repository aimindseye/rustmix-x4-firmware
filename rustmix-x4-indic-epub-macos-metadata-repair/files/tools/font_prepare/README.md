...
When caches are written directly to a FAT-formatted SD card on macOS, Finder or the filesystem may create AppleDouble `._*` files and `.DS_Store`. The preparation and validation commands remove these host-only metadata files automatically before inspecting the cache. They are not Reader assets.

## Troubleshoot generation failures

The wrapper streams Cargo and generator output while retaining the same text for post-generation metadata parsing. If preparation fails, it preserves the extracted EPUB `BOOK.TXT` work directory and prints its path. A cache generator success is followed automatically by page-local cache validation.
