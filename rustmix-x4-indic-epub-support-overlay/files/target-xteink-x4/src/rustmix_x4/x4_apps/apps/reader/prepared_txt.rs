...
fn page_local_file_name(prefix: char, page: usize, extension: &str) -> Result<String, PreparedTxtError> {
...
mod tests {
    ...
    fn generates_page_local_fat_83_names() {
        ...
        assert_eq!(page_local_font_file_name(FONT_DEVANAGARI, 1).unwrap(), "D00001.VFN");
        assert_eq!(page_local_font_file_name(FONT_GUJARATI, 1).unwrap(), "G00001.VFN");
    }

    #[test]
    fn parses_legacy_fonts_index_with_optional_script_fonts() {
        let fonts = parse_fonts_index(
