...
Indic shaping is prepared on a computer so the X4 can render Hindi, Sanskrit, Vedic-mark, and Gujarati text without running a large shaping engine on-device. Generate a cache with local Noto fonts, using the exact filename shown by the X4 file browser as `--device-path`:
...
Keep the original book at the exact `--device-path`. New caches use page-local 8.3-safe assets (`P00000.VRN`, `L00000.VFN`, `D00000.VFN`, and optional `G00000.VFN`). Gujarati font assets are generated only for pages that use Gujarati glyphs.
