...
Each page loads only its required font assets. This avoids retaining whole-book Devanagari or Gujarati glyph sets in the X4 heap. The firmware still accepts older `FONTS.IDX` + `PAGES.IDX` smoke caches for backward compatibility.

## Font prerequisites

Use locally installed or downloaded Google Noto fonts:
...
Font files are inputs on your computer. They are not committed into this repository.
