...
fn run() -> Result<(), String> {
    ...
    fs::create_dir_all(&cache_dir).map_err(|err| format!("create staging cache directory: {err}"))?;
...
fn page_local_file_name(prefix: char, page: usize, extension: &str) -> Result<String, String> {
    ...
    let stem = String::from_utf8(digits.to_vec()).map_err(|_| "base36 encode failed".to_string())?;
...
fn replace_cache_directory(staged: &Path, final_dir: &Path, backup: &Path) -> Result<(), String> {
    ...
    if had_previous {
        fs::rename(final_dir, backup).map_err(|err| format!("move previous cache to backup: {err}"))?;
    }
